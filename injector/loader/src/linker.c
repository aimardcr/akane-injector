...
static void _linker_call_destructors(struct csoloader_elf *img) {
  /* Cross-process: fini_array / fini_func live at target VAs, unreachable
   * from the controller. Symmetric with the constructor skip in linker_link. */
  if (img->target_base != img->base) {
    LOGD("Cross-process: skipping destructors for %s", img->elf);
    return;
  }
...
static int _linker_protect_gnu_relro(struct csoloader_elf *img) {
  ...
  /* Operate on storage-relative offsets so the call routes correctly
   * through the mem_ops backend (cross-process backends defer to commit). */
  struct csoloader_mem_ops *ops = img->mem_ops ? img->mem_ops : &csoloader_local_mem_ops;
  ...
  for (int i = 0; i < img->header->e_phnum; i++) {
    ...
    ElfW(Addr) seg_page_start_off = _page_start(phdr[i].p_vaddr - img->bias);
    ElfW(Addr) seg_page_end_off   = _page_end(phdr[i].p_vaddr + phdr[i].p_memsz - img->bias);
    size_t seg_size = seg_page_end_off - seg_page_start_off;

    if (seg_size == 0) continue;

    int ret = ops->protect(img->mem_ctx, img->base, seg_page_start_off, seg_size, PROT_READ);
    if (ret < 0) {
      LOGW("Failed to protect GNU_RELRO at off 0x%llx (size %zu) in %s: %s",
        (unsigned long long)seg_page_start_off, seg_size, img->elf, strerror(errno));

      return -1;
    }

    LOGD("Protected GNU_RELRO region at off 0x%llx (size %zu) in %s",
      (unsigned long long)seg_page_start_off, seg_size, img->elf);
  }
...
static void _linker_release_dependency(struct linker *linker, int index, bool unload) {
  ...
  struct csoloader_mem_ops *dep_ops = dep->mem_ops ? dep->mem_ops : &csoloader_local_mem_ops;
  struct csoloader_mem_ctx *dep_ctx = dep->mem_ctx;
  ...
  if (dep->is_manual_load && dep_map_size > 0) {
    if (unload) {
      dep_ops->release(dep_ctx, dep_base, dep_map_size);
    } else if (dep_ops->detach) {
      /* Abandon: drop bookkeeping, leave the loaded image alive. */
      dep_ops->detach(dep_ctx, dep_base, dep_map_size);
    }
  }
}
...
void linker_destroy(struct linker *linker) {
  ...
  struct csoloader_mem_ops *ops = linker->mem_ops ? linker->mem_ops : &csoloader_local_mem_ops;
  struct csoloader_mem_ctx *main_ctx = linker->main_mem_ctx;
  ...
  if (main_base && main_map_size > 0)
    ops->release(main_ctx, main_base, main_map_size);

  linker->dep_count = 0;
  linker->is_linked = false;
  linker->main_map_size = 0;
  linker->mem_ops = NULL;
  linker->main_mem_ctx = NULL;
}

/* INFO: Free resources related to the library without unloading it */
void linker_abandon(struct linker *linker) {
  void *main_base = linker->img ? linker->img->base : NULL;
  size_t main_map_size = linker->main_map_size;
  struct csoloader_mem_ops *ops = linker->mem_ops ? linker->mem_ops : &csoloader_local_mem_ops;
  struct csoloader_mem_ctx *main_ctx = linker->main_mem_ctx;
  ...
  /* Drop our backend-side bookkeeping (mirror, ctx). The loaded image
   * itself stays alive in its address space (target VMA in cross-process
   * mode, or current mmap in local mode). */
  if (main_base && main_map_size > 0 && ops->detach)
    ops->detach(main_ctx, main_base, main_map_size);
...
static int _linker_load_one_segment(struct csoloader_mem_ops *ops,
                                    struct csoloader_mem_ctx *mem_ctx,
                                    void *base,
                                    int fd, ElfW(Phdr) *phdr, ElfW(Addr) bias,
                                    off_t file_off) {
  ...
  /* Offsets within the reserved region. */
  size_t page_storage_off = (size_t)(page_start - (ElfW(Addr))(uintptr_t)base);
  ...
  if (file_len > 0 && ops->map_segment(mem_ctx, base, fd, file_off + file_page,
                                       page_storage_off, file_len, prot) != 0) {
    PLOGE("map_segment file-backed");
  ...
  if (page_end > page_start + file_len) {
    ...
    size_t bss_off  = page_storage_off + file_len;

    if (ops->map_zero(mem_ctx, base, bss_off, bss_size, prot) != 0) {
      PLOGE("map_zero BSS");
  ...
  if (needs_mprotect && ops->protect(mem_ctx, base, page_storage_off,
                                     page_end - page_start, prot | PROT_EXEC) != 0) {
    PLOGE("protect to add PROT_EXEC");
...
void *linker_load_library_manually(const char *lib_path,
                                   struct csoloader_mem_ops *mem_ops,
                                   struct loaded_dep *out) {
  _linker_internal_init();

  if (!mem_ops) mem_ops = &csoloader_local_mem_ops;
  struct csoloader_mem_ctx *mem_ctx = NULL;
  ...
  void *target_base = NULL;
    ...
    if (mem_ops->reserve(mem_ops, hint, out->map_size, &mem_ctx, &base, &target_base) != 0) {
  ...
  #else
    if (mem_ops->reserve(mem_ops, NULL, out->map_size, &mem_ctx, &base, &target_base) != 0) {
  ...
  for (int i = 0; i < eh.e_phnum; i++) {
    ...
    if (_linker_load_one_segment(mem_ops, mem_ctx, base, fd, &phdr[i], bias, 0) != 0) {
      LOGE("Failed to load segment %d of %s", i, lib_path);

      mem_ops->release(mem_ctx, base, out->map_size);
      close(fd);
...
static bool _linker_process_unified_relocation(struct linker *linker, struct loaded_dep *dep, struct _linker_unified_r *r, ElfW(Sym) *dynsym, char *dynstr, bool is_rela) {
  /* write_bias indexes into storage (mirror in cross-process; same as
   * load_bias in local). target_bias is the runtime VA bias used for
   * computing the values being written. */
  ElfW(Addr) write_bias  = (ElfW(Addr))dep->img->base        - dep->img->bias;
  ElfW(Addr) target_bias = (ElfW(Addr))dep->img->target_base - dep->img->bias;
  /* For brevity below we keep the variable name `load_bias` referring to
   * what the value should resolve to (target_bias) and use `target_addr`
   * for the storage write address (write_bias-derived). */
  ElfW(Addr) load_bias = target_bias;
  ElfW(Addr) *target_addr = (ElfW(Addr) *)(write_bias + r->r_offset);
...
static bool _linker_process_relocations(struct linker *linker, struct loaded_dep *dep) {
  ...
  if (relr) {
    ...
    /* RELR rebases each location by load_bias. With cross-process loading,
     * write the location through `write_bias` (storage) but adjust the
     * value by `target_bias` (runtime VA in target). They're equal in
     * local mode. */
    ElfW(Addr) write_bias  = (ElfW(Addr))dep->img->base        - dep->img->bias;
    ElfW(Addr) target_bias = (ElfW(Addr))dep->img->target_base - dep->img->bias;
    ...
    for (size_t i = 0; i < relr_count; i++) {
      ...
      if ((entry & 1) == 0) {
        ...
        ElfW(Addr) *target_addr = (ElfW(Addr) *)(write_bias + reloc_offset);
        *target_addr += target_bias;
      ...
      for (size_t bit = 0; bitmap != 0 && bit < bits_per_entry - 1; bit++, bitmap >>= 1) {
        ...
        ElfW(Addr) *target_addr = (ElfW(Addr) *)(write_bias + base_offset + (bit * sizeof(ElfW(Addr))));
        *target_addr += target_bias;
  ...
  if (rela) {
    ...
    for (size_t i = 0; i < rela_sz / rela_ent; ++i) {
      ...
      if (!_linker_process_unified_relocation(linker, dep, &unified_r, dynsym, dynstr, true)) return false;
    }
  }
  ...
  if (rel) {
    ...
    for (size_t i = 0; i < rel_sz / rel_ent; ++i) {
      ...
      if (!_linker_process_unified_relocation(linker, dep, &unified_r, dynsym, dynstr, false)) return false;
    }
  }
    ...
    if (android_reloc) {
      ...
      for (uint64_t i = 0; i < num_relocs; ) {
        ...
        for (size_t j = 0; j < group_size; ++j) {
          ...
          if (!_linker_process_unified_relocation(linker, dep, &unified_r, dynsym, dynstr, is_rela)) return false;
        }
...
static void _linker_restore_protections(struct csoloader_elf *image) {
  ...
  struct csoloader_mem_ops *ops = image->mem_ops ? image->mem_ops : &csoloader_local_mem_ops;
  for (size_t i = 0; i < num_pages; i++) {
    uintptr_t current_page = start_page_addr + (i * system_page_size);
    int final_prot = page_protections[i];
    size_t off = current_page - (uintptr_t)image->base;

    if (final_prot != 0 && ops->protect(image->mem_ctx, image->base, off, system_page_size, final_prot) != 0) {
      LOGW("protect failed to restore prot %d for page off 0x%zx in %s: %s",
        final_prot, off, image->elf, strerror(errno));
    } else if ((final_prot & PROT_EXEC) && (final_prot & PROT_READ) && image->mem_ops == NULL) {
      /* Only flush icache when the bytes live in our address space. */
...
bool linker_link(struct linker *linker) {
  ...
  for (size_t i = 0; i < carray_length(loaded_libs); i++) {
    ...
    /*
     * Cross-process: locate the dep in the TARGET's address space first.
     * Prefer the controller-loaded copy (via dl_iterate_phdr) for img->base
     * so that IFUNC resolvers can be invoked against an executable mapping;
     * the resolver result is then translated to the target VA in
     * handle_indirect_symbol. Falls back to a PROT_READ file mmap when the
     * dep is not loaded in the controller -- IFUNCs on that path will fail.
     */
    struct csoloader_elf *check_img = NULL;
    if (linker->mem_ops && linker->mem_ops->find_dep_target) {
      char dep_path[PATH_MAX];
      void *dep_target_base = NULL;
      if (linker->mem_ops->find_dep_target(linker->mem_ops, lib_name,
                                           dep_path, sizeof(dep_path),
                                           &dep_target_base) == 0) {
        check_img = csoloader_elf_create(lib_name, NULL);
        if (check_img && check_img->base) {
          check_img->target_base = dep_target_base;
          LOGD("Resolved dep %s in target at %p (controller-loaded base %p)",
               lib_name, dep_target_base, check_img->base);
        } else {
          if (check_img) {
            csoloader_elf_destroy(check_img);
            check_img = NULL;
          }
          int dfd = open(dep_path, O_RDONLY | O_CLOEXEC);
          if (dfd >= 0) {
            struct stat dst;
            if (fstat(dfd, &dst) == 0 && dst.st_size > 0) {
              void *file_map = mmap(NULL, dst.st_size, PROT_READ,
                                    MAP_PRIVATE, dfd, 0);
              if (file_map != MAP_FAILED) {
                check_img = csoloader_elf_create(dep_path, file_map);
                if (check_img) {
                  check_img->target_base = dep_target_base;
                  LOGD("Resolved dep %s in target at %p (file: %s)",
                       lib_name, dep_target_base, dep_path);
                } else {
                  munmap(file_map, dst.st_size);
                }
              }
            }
            close(dfd);
    ...
    /* Fall back to the in-process dl_iterate_phdr lookup. In cross-process
     * mode this is wrong (returns controller addresses) but lets in-process
     * loads keep working. */
    if (!check_img)
      check_img = csoloader_elf_create(lib_name, NULL);
    ...
    if (check_img && check_img->base) {
      ...
      void *base_addr = linker_load_library_manually(lib_full_path, linker->mem_ops, current_dep);
      if (!base_addr) {
      ...
      current_dep->img->target_base =
        current_dep->target_base ? current_dep->target_base : base_addr;
      current_dep->img->mem_ops = current_dep->mem_ops;
      current_dep->img->mem_ctx = current_dep->mem_ctx;
  ...
  struct csoloader_mem_ops *main_ops = linker->img->mem_ops ? linker->img->mem_ops : &csoloader_local_mem_ops;
  for (int j = 0; j < linker->img->header->e_phnum; j++) {
    if (main_phdr[j].p_type != PT_LOAD || (main_phdr[j].p_flags & PF_W)) continue;

    size_t page_off = _page_start(main_phdr[j].p_vaddr - linker->img->bias);
    size_t page_len = _page_end(main_phdr[j].p_vaddr + main_phdr[j].p_memsz) - _page_start(main_phdr[j].p_vaddr);
    int new_prot = PROT_READ | PROT_WRITE | (main_phdr[j].p_flags & PF_X ? PROT_EXEC : 0);

    if (main_ops->protect(linker->img->mem_ctx, linker->img->base, page_off, page_len, new_prot) != 0)
      LOGW("protect failed to make main image segment %d writable: %s", j, strerror(errno));
  }
  ...
  for (int i = 0; i < linker->dep_count; i++) {
    ...
    struct csoloader_mem_ops *dep_ops = dep->img->mem_ops ? dep->img->mem_ops : &csoloader_local_mem_ops;
    for (int j = 0; j < dep->img->header->e_phnum; j++) {
      if (dep_phdr[j].p_type != PT_LOAD || (dep_phdr[j].p_flags & PF_W)) continue;

      size_t page_off = _page_start(dep_phdr[j].p_vaddr - dep->img->bias);
      size_t page_len = _page_end(dep_phdr[j].p_vaddr + dep_phdr[j].p_memsz) - _page_start(dep_phdr[j].p_vaddr);
      int new_prot = PROT_READ | PROT_WRITE | (dep_phdr[j].p_flags & PF_X ? PROT_EXEC : 0);

      if (dep_ops->protect(dep->img->mem_ctx, dep->img->base, page_off, page_len, new_prot) != 0)
        LOGW("protect failed for make segment %d in %s writable: %s", j, dep->img->elf, strerror(errno));
    }
  }
  ...
  /* Backend commit: cross-process backends push the mirror to target here
   * with all final protections applied. Local backend's commit is a no-op. */
  {
    struct csoloader_mem_ops *m_ops = linker->img->mem_ops ? linker->img->mem_ops : &csoloader_local_mem_ops;
    if (m_ops->commit && m_ops->commit(linker->img->mem_ctx, linker->img->base, linker->main_map_size) != 0) {
      LOGE("commit failed for main image %s", linker->img->elf);
      return false;
    }
    for (int i = 0; i < linker->dep_count; i++) {
      struct loaded_dep *dep = &linker->dependencies[i];
      if (!dep->is_manual_load) continue;
      struct csoloader_mem_ops *d_ops = dep->img->mem_ops ? dep->img->mem_ops : &csoloader_local_mem_ops;
      if (d_ops->commit && d_ops->commit(dep->img->mem_ctx, dep->img->base, dep->map_size) != 0) {
        LOGE("commit failed for dep %s", dep->img->elf);
        return false;
      }
    }
  }
  ...
  /* Cross-process load: constructors live in the target's address space
   * and would crash if invoked from the controller. Skip them. */
  bool cross_process = (linker->img->target_base != linker->img->base);
  if (cross_process) {
    LOGD("Cross-process load: skipping constructor calls");
