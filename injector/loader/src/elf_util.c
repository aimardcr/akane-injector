...
static int dl_cb(struct dl_phdr_info *info, size_t size, void *data) {
  ...
  if (strstr(info->dlpi_name, img->elf)) {
    ...
    img->target_base = (void *)info->dlpi_addr;
...
struct csoloader_elf *csoloader_elf_create(const char *elf, void *base) {
  ...
  if (base) {
    ...
    */
    img->base = base;
    img->target_base = base;     /* default; csoloader_load may overwrite */
...
ElfW(Addr) csoloader_elf_symb_address_exported(struct csoloader_elf *img, const char *name) {
  ...
  /* Returned to caller as a runtime VA (relocation value, dlsym-style
   * lookup, etc.). Use target_base so cross-process loads see the right
   * address; in local mode target_base == base so behavior is unchanged. */
  return (ElfW(Addr))((uintptr_t)img->target_base + offset - img->bias);
}
...
static ElfW(Addr) handle_indirect_symbol(struct csoloader_elf *img, ElfW(Off) offset) {
  ...
  ElfW(Addr) result;
    ...
    result = ((ifunc_resolver_t)resolver_addr)(args._hwcap | _IFUNC_ARG_HWCAP, &args);
  #elif defined(__arm__)
      typedef ElfW(Addr) (*ifunc_resolver_t)(unsigned long);

      result = ((ifunc_resolver_t)resolver_addr)(getauxval(AT_HWCAP));
  #elif defined(__riscv)
    typedef ElfW(Addr) (*ifunc_resolver_t)(uint64_t, __riscv_hwprobe_t, void *);

    result = ((ifunc_resolver_t)resolver_addr)(getauxval(AT_HWCAP), __riscv_hwprobe, NULL);
  #else
    typedef ElfW(Addr) (*ifunc_resolver_t)(void);

    result = ((ifunc_resolver_t)resolver_addr)();
  #endif

  /* Cross-process: resolver ran against the controller-loaded copy of the
   * dep and returned a controller-side VA. Translate to the target's VA so
   * the relocation site stores an address that's valid in the target. In
   * local mode target_base == base and this is a no-op. */
  if (img->target_base != img->base && result != 0)
    result = (ElfW(Addr))((uintptr_t)result - (uintptr_t)img->base + (uintptr_t)img->target_base);
