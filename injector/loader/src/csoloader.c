...
/* Verbosity for the akane logging fork; 0 = errors+warnings only,
 * 1 = +info, 2 = +debug. The injector raises this from -v / -vv. */
int g_csoloader_verbose = 0;
...
bool csoloader_load(struct csoloader *lib, const char *lib_path) {
  struct csoloader_mem_ops *ops = lib->mem_ops ? lib->mem_ops : &csoloader_local_mem_ops;
  struct loaded_dep dep_info = { 0 };
  void *map_start = linker_load_library_manually(lib_path, ops, &dep_info);
  if (!map_start) {
  ...
  if (!elf_image) {
    ...
    if (dep_info.map_size > 0)
      ops->release(dep_info.mem_ctx, map_start, dep_info.map_size);

    return false;
  }

  /* In cross-process mode the backend gave us a separate target VA;
   * fall back to mirror (== map_start) for in-process loads. */
  elf_image->target_base = dep_info.target_base ? dep_info.target_base : map_start;
  elf_image->mem_ops     = dep_info.mem_ops;
  elf_image->mem_ctx     = dep_info.mem_ctx;
  ...
  lib->linker.mem_ops       = ops;
  lib->linker.main_mem_ctx  = dep_info.mem_ctx;
