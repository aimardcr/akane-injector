...
struct csoloader_elf {
  char *elf;
  /* Storage pointer: where the loaded image's bytes actually live in this
   * process. In local mode this is also the runtime VA. In cross-process
   * mode it's a mirror buffer in the controller; target_base holds the
   * runtime VA in the target. All bookkeeping pointers below
   * (dynsym_start, gnu_bucket_, eh_frame, etc.) are derived from `base`
   * since they are read from this process. */
  void *base;
  /* Runtime VA used for symbol math: load_bias = target_base - bias. In
   * local mode equals `base`. In cross-process mode it's the address in
   * the target process where this image will live after push. Symbols
   * returned to the user (csoloader_get_symbol) and values written into
   * relocations are computed against this. */
  void *target_base;
  /* Memory backend that owns this image's pages. NULL = no manual load
   * (e.g. resolved via dl_iterate_phdr); protect() must not be called. */
