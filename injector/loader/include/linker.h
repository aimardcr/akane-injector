...
struct loaded_dep {
  ...
  void *map_base;
  void *target_base;     /* Runtime VA in target. Equals map_base in local mode. */
  size_t map_size;
...
struct linker {
  ...
  /* Memory backend for the main library. Recursively loaded deps inherit
   * via dep->mem_ops -- they store their own pointer for symmetry. */
  struct csoloader_mem_ops *mem_ops;
  struct csoloader_mem_ctx *main_mem_ctx;
};
